...
def main():
    ...
    # Создание окна
    window = glfw.create_window(800, 600, "OpenGL Renderer", None, None)
    if not window:
        glfw.terminate()
        return
    ...
    while not glfw.window_should_close(window):
        ...
        angle = current_time * math.pi / 3  # π/3 рад/сек

        # Матрица вращения вокруг Z
        model = np.identity(4, dtype=np.float32)
        model[0, 0] = math.cos(angle)
        model[0, 1] = -math.sin(angle)
        model[1, 0] = math.sin(angle)
        model[1, 1] = math.cos(angle)
